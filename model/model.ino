...
void loop() {
  ...
  NPKFunc();
  moisture_int = analogRead(moisture_sensor)/1023;
//  moisture_percentage=map(moisture_per,0,100,20,290);
  moisture_percentage=100-(moisture_int*100);
  ...
  if (Signal==1)
  {
    ...
    if ((WiFi.status() == WL_CONNECTED)) { //Check the current connection status
      WiFiClient client;
      HTTPClient http;
      String query_api="http://192.168.29.211:8000/api/data/?n="+String(nitro)+"&p="+String(phos)+"&k="+String(potas)+"&t="+String(tempC)+"&h="+String(hum);
      ...
      if (httpCode > 0) { //Check for the returning code
        ...
        display_serial(nitro, phos, potas, hum, tempC, moisture_percentage,moisture_int);
...
void display_serial(byte nitro , byte phos, byte potas, float hum, float tempC, float moisture_percentage,float moisture_int){
